...
func DrawFunction(inputX, inputY []float64, text string) *plot.Plot {
	...
	for x := range inputX {
		points = append(points, plotter.XY{X: inputX[x], Y: inputY[x]})
	}
...
func DrawFunction2(inputX, inputY []float64, text string, n *plot.Plot) *plot.Plot {
	// Создаем новый график
	p := plot.New()
